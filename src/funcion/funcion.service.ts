...
export class FuncionService {
    ...
    async getFuncionesByPeliculaId(
        ...
        for (const funcion of funciones) {
            let todasButacasOcupadas = true;
            if (funcion.estaDisponible) {
                for (const disponibilidad of funcion.disponibilidadButaca) {
                    if ((disponibilidad.estadoDisponibilidadButaca.nombre === EstadoButacaEnum.DISPONIBLE)
                        ||
                        (disponibilidad.estadoDisponibilidadButaca.nombre === EstadoButacaEnum.RESERVADA)) {
                        todasButacasOcupadas = false;
                    }
                }
                if (todasButacasOcupadas) {
                    funcion.estaDisponible = false;
            ...
            return await Promise.all(
                funciones.map(async (funcion) => {
                    ...
                    return {
                        id: funcion.id,
                        peliculaId: funcion.peliculaId,
                        peliculaNombre: peliculaResponse.data.titulo,
                        fecha: funcion.fecha,
                        hora: funcion.hora,
                        estaDisponible: funcion.estaDisponible,
