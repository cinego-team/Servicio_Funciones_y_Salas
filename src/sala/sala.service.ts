...
export class SalaService {
        ...
        try {
            ...
            return salas.map((sala) => {
                const response: SalaResponse = {
                    ...
                    cantButacasPorFila: sala.filas.length > 0 ? sala.filas[0].butacas.length : 0,
                };
                return response;
            });
        } catch (error) {
            throw new InternalServerErrorException('Error al obtener las salas');
        }
    }
    ...
    async update(id: number, data: Partial<SalaInput>): Promise<SalaResponse> {
        try {
            ...
            if (data.cantFilas && !data.cantButacasPorFila) {
                throw new InternalServerErrorException('Debe proporcionar cantButacasPorFila al actualizar cantFilas');
            }

            if (!data.cantFilas && data.cantButacasPorFila) {
                const filasACrear: Fila[] = await this.filaService.createMultiple(sala.filas.length, data.cantButacasPorFila);
                await this.filaService.saveArray(filasACrear);
            }
    ...
    async getButacasIdBySalaId(salaId: number): Promise<number[]> {
        try {
            ...
            throw new InternalServerErrorException('Error al obtener las butacas de la sala');
        }
    }
}