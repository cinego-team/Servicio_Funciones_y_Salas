...
import {
    ...
    EstadoButacaEnum,
} from '../entities/estadoDisponibilidadButaca.entity';
...
export class DisponibilidadButacaService {
    ...
    createDisponibilidadButaca(data: {
        ...
        try {
            ...
            throw new InternalServerErrorException(
                'Error al crear la disponibilidad de butaca',
            );
        }
    }

    async saveArray(
        disponibilidades: DisponibilidadButaca[],
    ): Promise<DisponibilidadButaca[]> {
        try {
            return await this.disponibilidadRepo.save(disponibilidades);
        } catch (error) {
            throw new InternalServerErrorException(
                'Error al guardar las disponibilidades de butaca',
            );
        }
    }
    ...
    async findAll(): Promise<DisponibilidadButacaResponse[]> {
        try {
            const disponibilidades = await this.disponibilidadRepo.find({
                relations: ['funcion', 'butaca', 'estadoDisponibilidadButaca'],
            });
            ...
            const response: DisponibilidadButacaResponse[] =
                disponibilidades.map((d) => ({
                    ...
                    estadoDisponibilidadButacaId: d.estadoDisponibilidadButaca
                        .nombre as EstadoButacaEnum,
                }));
            return response;
        } catch (error) {
            throw new InternalServerErrorException(
                'Error al obtener las disponibilidades de butaca',
            );
        }
    }
    ...
    async findOne(id: number): Promise<DisponibilidadButacaResponse> {
        try {
            ...
            const response: DisponibilidadButacaResponse = {
                ...
                estadoDisponibilidadButacaId: disponibilidad
                    .estadoDisponibilidadButaca.nombre as EstadoButacaEnum,
            };
            return response;
        } catch (error) {
            throw new InternalServerErrorException(
                'Error al obtener la disponibilidad de butaca',
            );
        }
    }
    ...
    async update(
        id: number,
        data: Partial<DisponibilidadButacaInput>,
        ...
        if (data.funcionId) {
            const funcion: Funcion = await this.funcionService.getFuncionById(
                data.funcionId,
            );
        ...
        if (data.butacaId) {
            const butaca: Butaca = await this.butacaService.getButacaById(
                data.butacaId,
            );
        ...
        if (data.estadoDisponibilidadButacaId) {
            const estado: EstadoDisponibilidadButaca =
                await this.estadoDisponibilidadService.getEstadoByNombre(
                    data.estadoDisponibilidadButacaId,
                );
        ...
        try {
            ...
            if (!estadoDisponible) {
                throw new NotFoundException(
                    'Estado DISPONIBLE no encontrado en la base de datos',
                );
            }

            const estadoReservada =
                await this.estadoDisponibilidadService.getEstadoByNombre(
                    EstadoButacaEnum.RESERVADA,
                );

            if (!estadoReservada) {
                throw new NotFoundException(
                    'Estado RESERVADA no encontrado en la base de datos',
                );
            }
            ...
            if (actualizadas === 0) {
                throw new NotFoundException(
                    'No se encontraron disponibilidades para reservar',
                );
            }
            for (const disponibilidad of disponibilidades) {
                if (
                    disponibilidad.estadoDisponibilidadButaca.id !==
                    estadoDisponible.id
                ) {
                    throw new NotFoundException(
                        `La disponibilidad con id ${disponibilidad.id} no está en estado DISPONIBLE`,
                    );
                }
            }
    ...
    async ocuparButacas(
        ...
        try {
            ...
            const estadoOcupada =
                await this.estadoDisponibilidadService.getEstadoByNombre(
                    EstadoButacaEnum.OCUPADA,
                );

            if (!estadoOcupada) {
                throw new NotFoundException(
                    'Estado OCUPADO no encontrado en la base de datos',
                );
            }
    ...
    async remove(id: number): Promise<void> {
        try {
            ...
            throw new InternalServerErrorException(
                'Error al eliminar la disponibilidad de butaca',
            );
        }
    }
    ...
    async getDisponibilidadById(id: number): Promise<DisponibilidadButaca> {
        try {
            ...
            if (!disponibilidad) {
                throw new NotFoundException(
                    `Disponibilidad de butaca con id ${id} no encontrada`,
                );
            }
            return disponibilidad;
        } catch (error) {
            throw new InternalServerErrorException(
                'Error al obtener la disponibilidad de butaca por ID',
            );
        }
    }
}
